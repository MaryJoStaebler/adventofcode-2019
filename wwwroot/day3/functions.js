...
function createWireCoords(arrWire)
{
    let currX =0;
    let currY =0;
    let arrCoords = [];
    for(let i =0; i < arrWire.length;i++){
        let regExp = /([RDUL])(\d+)/gi;
        let arrInstructions = regExp.exec(arrWire[i]);
        let direction = arrInstructions[1];
        let steps = arrInstructions[2];
        ...
        for(let j = 1;j <= steps;j++){
            ...
            //arrCoords.push(new Point(currX,currY));
            arrCoords.push(currX+","+currY);
        }
    }
    return arrCoords;
}

let arrWireACoords = createWireCoords(arrWireA);
let arrWireBCoords = createWireCoords(arrWireB);

// console.log("A");
// console.log(arrWireACoords);
// console.log("B");
// console.log(arrWireBCoords);

let arrMatchPoints = [];
function optionOneForGoal1() {
    let endResultGoal1;

    for(let a = 0; a < arrWireACoords.length; a++){

        for(let b = 0; b < arrWireBCoords.length; b++){
            if(arrWireACoords[a] === arrWireBCoords[b]) {
                console.log("match: "+arrWireACoords[a]);
                let arrPoints = arrWireACoords[a].split(",");
                let dist =Math.abs(arrPoints[0])+Math.abs(arrPoints[1]);
                arrMatchPoints.push(new Point(arrWireACoords[a]));
                if(endResultGoal1 === undefined ||  endResultGoal1 > dist)
                {
                    endResultGoal1 = dist;
                }
            }
    ...
    //localStorage.setItem("matches",arrMatchPoints);
    console.log("End Result Goal 1: "+endResultGoal1);
}

/* Faster at Goal 1 */
function optionTwoForGoal1 ()
{
    let setA = new Set(arrWireACoords);
    let setB = new Set(arrWireBCoords);
    ...
    for (let point of intersection) {
        let arrPoints = point.split(",");
        let dist =Math.abs(arrPoints[0])+Math.abs(arrPoints[1]);
        if (shortestPath === undefined || shortestPath > dist)
        {
            shortestPath = dist;
        }
    }
    console.log("End Result Goal 1: "+shortestPath);
}

// loop through an array and count how many nodes we take to get to a spot
// on intersect coord array, write how many steps for a wire to that point
// loop through intersect array and find lowest sum of both
// remember to take first node (0,0) in the step count

function findSteps(arrCoords,arrMatches,strProp)
{
    for(let i = 0; i < arrCoords.length; i++)
    {
        if(arrMatches.coords === arrCoords[i])
        {
            arrMatches[strProp] = i+1;
            break;
        }
    }
}
...
for(let m = 0; m < arrMatchPoints.length; m++){

    findSteps(arrWireACoords,arrMatchPoints[m],"stepsForA");
    findSteps(arrWireBCoords,arrMatchPoints[m],"stepsForB");
    let result = arrMatchPoints[m].stepsForB+arrMatchPoints[m].stepsForA;
    console.log(arrMatchPoints[m]);
    if(endResultStep2 === undefined || endResultStep2 > result)
    {
        endResultStep2 = result;
    }
    console.log("result is: "+endResultStep2)
}